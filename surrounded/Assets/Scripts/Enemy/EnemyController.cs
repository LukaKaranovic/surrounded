...
namespace Enemy
{
    public class EnemyController : MonoBehaviour
    {
        ...
        public float
            targetInnerRadius = 7,
            targetOuterRadius = 10; //inner and outer radius of target ring to move to around player
        ...
        {
            if (Vector2.Distance(transform.position, player.transform.position + moveTarget) <
                1) //Enemy is near target position
            {
                moveTarget = FindPointNearPlayer();
            }
        ...
        protected Vector3 avoidanceAdjustment(Vector3 heading)
        {
            ...
            foreach (Collider2D collider in NearbyColliders)
            {
                if (collider.gameObject != this.gameObject && !collider.gameObject.CompareTag("Bullet"))
                {
                    ...
                    heading += avoidanceRadius / distance *
        ...
        protected Vector3 FindPointNearPlayer() // changed from private to protected
        {
            Vector2
                randomDirection =
                    Random.insideUnitCircle.normalized; //vector pointing towards direction of target around player
