...
Since the inclusion of bosses and boss rounds was decided on late into the requirements stage, a lot of the details surrounding bosses are vague and subject to change. This is something we hope to have fully fledged out in phase 3 when we do revisions to this document.
...
As the game needs to be playtested to know if it is balanced and if values are appropriate, a lot of the values and scaling is subject to change. This is stated in a disclaimer at the top of the key features and behaviour section.

The same goes with the UI likely not being in its final form and layout, as playtesting may lead to finding issues with the user experience. A disclaimer has been put in the user interface and navigation section.

Please note that we are working strictly on proof of concept, no real coding has been finalised. 
...
## <a name="target"></a>Target audience and motivation
...
The goal of Surrounded is to survive the onslaught of enemy alien ships for as long as possible. There is no way to “win” and instead success is measured by a score that is presented in the top right of the screen. Score is gained in smaller amounts by killing enemies and larger amounts upon completing rounds and defeating bosses (see the Round System and Enemy Design sections for specific amounts).
...
### Map and Terrain
The map will be set and not randomly generated, having floating asteroids and planets scattered around the open space (having a randomly generated map is a stretch goal).
...
Rocket Boosters: Increases movement speed by 10% (+10% per stack)
Machine Guns: Fire Rate increases increases by 10% (+5% per stack)
Diverge: Player fires three bullets each time, fired shots will expand in a cone-based pattern towards the cursor (+10% damage per stack)
...
## <a name="ui"></a>User interface and navigation 
...
Directly to the right of the players health bar will be text labelled as “Score: ” which will show the players current score as an integer value to the right of it. The score will increase depending on the player's actions as discussed in the gameflow section.
...
## <a name="uses"></a>Use cases/scenarios 
...
## <a name="nonfunc"></a>Non-functional requirements 
...
Bullet-mayhem: Chaotic shooter genre
